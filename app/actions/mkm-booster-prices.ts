...
function generateCandidateUrls(vendor: string | null, edition: string | null, mainType: string | null): string[] {
	...
	// e.g. https://www.cardmarket.com/en/Magic/Products/Boosters/Dominaria-Collector-Booster
	candidates.push(`https://www.cardmarket.com/en/${gamePath}/Products/Boosters/${combinedSlug}`);

	// --- CANDIDATE 2: Magic Fallback (Only if Magic) ---
	// e.g. https://www.cardmarket.com/en/Magic/Products/Boosters/Magic-The-Gathering-Dominaria-Collector-Booster
	if (gamePath === 'Magic') {
		candidates.push(`https://www.cardmarket.com/en/${gamePath}/Products/Boosters/Magic-The-Gathering-${combinedSlug}`);
	}
...
async function performLoginSequence() {
	...
	// Launch with puppeteer-real-browser
	const { browser, page } = await connect({
		headless: false, // Must be false for real-browser to work best
	...
	try {
		// Manual Authentication (Standard Puppeteer Method)
		...
		const loginState = await getPageState(page);
		if (loginState !== 'CLEAR') {
			console.error(`⚠️ Cloudflare/Queue on LOGIN page (State: ${loginState}). Waiting a bit...`);
			// RealBrowser handles turnstile, but if it's a queue, we might need to wait longer
		...
		await fs.writeFile(COOKIE_PATH, JSON.stringify(currentCookies, null, 2));
...
export async function fetchMarketData(productId: string, forceRefresh = false): Promise<ScrapeResult> {
	try {
		...
		if (existing?.cardmarketUrl) {
			...
			urlsToTry = generateCandidateUrls(product.vendor, product.edition, product.mainType);
		...
		// Map DB Language to MKM Tooltip String
		let mkmLangFilter: string | null = null;
		if (targetLanguage === 'Deutsch') mkmLangFilter = 'German';
		if (targetLanguage === 'English') mkmLangFilter = 'English';
		if (targetLanguage === 'Französisch') mkmLangFilter = 'French';
		...
		// 5. PREPARE SCRAPER
		console.log(`🕵️ [Scraper] Starting for ${productId} (Lang: ${mkmLangFilter})`);
			...
			while (!success && attempts < MAX_RETRIES) {
				...
				try {
					...
					if (!isValidProductPage) {
						...
						const isZeroResults = $('body').text().includes('No articles found');
					...
					$('.article-row').each((_, element) => {
						...
						console.log(`Row detected: Lang=${itemLang} | Filter=${mkmLangFilter}`);

						// Strict Filtering
						if (mkmLangFilter && itemLang !== mkmLangFilter) {
							return;
						}
						...
						if (!location) {
							...
							if (locMouseOver) {
								const match = locMouseOver.match(/Item location: (.+?)[`'"]\)/);
								if (match) location = match[1];
							}
						}
						location = location ? location.replace('Item location: ', '').trim() : '?';

						// 5. Extract Stock
						const countRaw = row.find('.amount-container .item-count').text().trim();
					...
					scrapedData = {
						...
						topListings: items.slice(0, 5)
					};

					console.log(`✅ Success! Found ${items.length} items (Filtered: ${mkmLangFilter || 'None'}) at ${targetUrl}`);
					usedUrl = targetUrl;
