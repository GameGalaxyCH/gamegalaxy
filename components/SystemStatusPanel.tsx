...
import { getGlobalSystemStatus, cancelOperation, getTotalOrderCount } from "@/app/actions/system-status";
...
export default function SystemStatusPanel() {
    ...
    useEffect(() => {
        ...
        // 2. Fetch "Goal" for Progress Bar (Total Orders)
        getTotalOrderCount().then(count => setTotalOrders(count));
    ...
    const renderActiveCard = (op: any, title: string, type: 'QUERY' | 'MUTATION') => {
        ...
        const isActive = isRunning || isCreated || isCancelling;

        return (
            <div className={`p-5 rounded-lg border flex flex-col justify-between transition-all relative overflow-hidden ${
                isActive ? "bg-blue-950/20 border-blue-900/40" : 
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-end gap-2">
                                    <span className="text-gray-200 font-mono text-lg">
                                        {parseInt(op.objectCount || 0).toLocaleString()}
                                    </span>
                                    {/* Show " / Total (%)" only for Queries where we know the total */}
                                    {type === 'QUERY' && totalOrders > 0 && (
                                        <span className="text-gray-500 text-xs mb-1">
                                            / {totalOrders.toLocaleString()} ({getProgress(parseInt(op.objectCount))}%)
                                        </span>
                                    )}
                                </div>
                                ...
                                {type === 'QUERY' && totalOrders > 0 && (
                                    <div className="w-full bg-gray-800 h-1.5 rounded-full mt-1.5 overflow-hidden">
                                        <div 
                                            ...
                                            style={{ width: `${getProgress(parseInt(op.objectCount))}%` }}
                                        ></div>
                                    </div>
                                )}
                            </div>
                        ...
                        {items.length === 0 ? (
                            ...
                            <tr key={op.id} className="hover:bg-gray-900/50 transition-colors">
                                ...
                                <td className="p-3 font-mono text-gray-300">
                                    {op.objectCount ? op.objectCount.toLocaleString() : '-'}
                                </td>
                                <td className="p-3 font-mono text-gray-300">
    ...
    return (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6 shadow-xl">
            ...
            {/* History Section */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 border-t border-gray-800/50 pt-6">
